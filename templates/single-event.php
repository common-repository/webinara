...
while ( have_posts() ) : the_post();
	...
	$webi_address = get_post_meta(get_the_ID(),'_webi_address',true);
	$webi_city = get_post_meta(get_the_ID(),'_webi_city',true);
	$webi_state = get_post_meta(get_the_ID(),'_webi_state',true);
	$webi_zipcode = get_post_meta(get_the_ID(),'_webi_zipcode',true);
	$webi_country = get_post_meta(get_the_ID(),'_webi_country',true);
    $webi_banner_theme = get_option('_webi_banner_theme');
	$webi_vanue = '';
	if(!empty($webi_address))
	{
		$webi_vanue = $webi_address;
	}
	if(!empty($webi_city))
	{
		if(empty($webi_vanue))
		{
			$webi_vanue = $webi_city;
		}
		else
		{
			$webi_vanue = $webi_vanue.', '.$webi_city;
		}
	}
	if(!empty($webi_zipcode))
	{
		if(empty($webi_vanue))
		{
			$webi_vanue = $webi_zipcode;
		}
		else
		{
			$webi_vanue = $webi_vanue.' '.$webi_zipcode;
		}
	}
	if(!empty($webi_state))
	{
		if(empty($webi_vanue))
		{
			$webi_vanue = $webi_state;
		}
		else
		{
			$webi_vanue = $webi_vanue.", ".$webi_state;
		}
	}
	
	if(!empty($webi_country))
	{
		if(empty($webi_vanue))
		{
			$webi_vanue = $webi_country;
		}
		else
		{			
			$webi_vanue = $webi_vanue.", ".$webi_country;
		}
	}
				...
				<div class="webi_body">
					<div class="webi_body_left">
						<div class="webi_subsec mobile_register register_btn">
							<a href="<?php echo get_post_meta(get_the_ID(),'_webi_registration_url',true); ?>" target="_blank"><?php esc_html_e('Register','webinara'); ?></a>
						</div>	
						<?php the_content(); 
						...
						if(count($webi_attachments) != 0 && !empty($webi_attachments))
						{
							echo '<div class="webi_subsec">
						...
						<div class="webi_body_right_content">
							...
							{								
								echo '<div class="webi_right_section webi_subsec">
									<h6>'.__('Venue:','webinara').'</h6>
									<div class="webi_vanue_section">
										<a href="https://maps.google.com/?q='.$webi_vanue.'" target="_blank">'.$webi_vanue.'</a>
									</div>	
								</div>';
							}
							...
							$webi_sponsar_arr = get_post_meta($post->ID, '_webi_sponser', true);	
							if(count($webi_sponsar_arr) != 0 && !empty($webi_sponsar_arr))
							{	
								$sponsor_count = 1;
								echo '<div class="webi_right_section webi_subsec">
								<h6>'.__('Sponsors:','webinara').'</h6>
